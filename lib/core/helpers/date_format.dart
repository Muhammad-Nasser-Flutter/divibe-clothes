...
class CustomDateFormat {
  static String formatDaysMonthsYears(String theTime, BuildContext context, {bool toLocal = false}) {
    ...
    final DateFormat formatter = DateFormat('dd-M-yyyy', context.locale.toString());
  ...
  static String MMMDDYYYYHMAMPM({required String theTime, BuildContext? context, bool toLocal = false}) {
    if (toLocal) {
      return DateFormat('MMM dd, yyyy - h:mm a', context?.locale.toString()).format(
        DateTime.parse(theTime).toLocal(),
      );
    } else {
      return DateFormat('MMM dd, yyyy - h:mm a', context?.locale.toString()).format(
  ...
  static String DMMMYYYYHHMMA({
    required String theTime,
    BuildContext? context,
    bool toLocal = false,
  }) {
    if (toLocal) {
      return DateFormat('d MMM yyyy, hh:mm a', context?.locale.toString()).format(
        DateTime.parse(theTime).toLocal(),
      );
    } else {
      return DateFormat('d MMM yyyy, hh:mm a', context?.locale.toString()).format(
